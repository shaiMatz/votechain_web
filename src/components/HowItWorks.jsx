...
const HowItWorks = ({ scroll }) => {
    ...
    useEffect(() => {
        ...
        if (containerRef.current) {
            ...
            containerRef.current.style.maxHeight = scroll === 'auto' ? '100lvh' : '100lvh';
        }
    ...
    const eaContent = (
        <div className="py-12 lg:p-8 pt-2 md:pt-14">
    ...
    return (
        <div ref={containerRef} className="px-2 max-h-screen min-h-screen">
            <div className="flex w-full h-[100lvh]">
                <div
                    {...voterHandlers}
                    className={`h-full transition-all duration-500 ease-in-out flex-grow border-r-2 relative overflow-y-auto lg:overflow-y-hidden cursor-pointer
                ...
                >
                    <div className={`absolute ${clickedSide === 'voter' ? 'top-10 left-1/2 transform -translate-x-1/2' : 'right-4 top-1/2 transform -translate-y-1/2'} text-2xl md:text-4xl font-pacifico flex flex-col`}>
                        ...
                        <div className='flex items-center gap-3'>
                            {clickedSide === 'voter' ? <p className='text-4xl'>Voter</p> : clickedSide === 'ea' ? "" : <p className={` ${clickedSide === 'ea' ? "text-lg" : ""}`}>I am a Voter</p>}
                            <FaUserCheck className="inline-block mr-2" />
                        </div>
                    </div>
                    {clickedSide === null ? <PiHandTap className="text-gray-700 absolute bottom-3 left-1/2 transform -translate-x-1/2 text-4xl" /> : ""}

                    {clickedSide === 'voter' && <div className="absolute top-20 left-1/2 transform -translate-x-1/2 text-lg">{voterContent}</div>}
                </div>
                <div
                    {...eaHandlers}
                    className={`h-full transition-all duration-500 ease-in-out flex-grow relative overflow-y-auto lg:overflow-y-hidden  cursor-pointer
               ...
               >
                    <div className={`absolute ${clickedSide === 'ea' ? 'top-10 right-1/2 transform translate-x-1/2' : 'left-4 top-1/2 transform -translate-y-1/2'}
                        ...
                        <div className='flex items-center gap-3'>
                            ...
                            {clickedSide === 'ea' ? <p className='text-4xl 
                            '>Election Administrator</p> : clickedSide === 'voter' ? "" : <p className={` ${clickedSide === 'voter' ? "text-lg" : ""}`}>I am an Election Administrator</p>}
                        </div>
